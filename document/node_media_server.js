...
class NodeMediaServer {
  ...
  run() {
    ...
    if (this.config.http) {
      ...
      this.nhs = new NodeHttpServer(this.config, function (req) {//TODO
        let urlInfo = URL.parse(req.url, true);//TODO
        let name = urlInfo.pathname.split(".flv")[0];//TODO
        name = name.split('/')[2];//TODO
        name = decodeURIComponent(name);//TODO
        that.nls.putOneRTMP({//TODO
          app: 'live',
          mode: 'static',
          edge: that.config.relay.edge + '/' + name,
          name: name
        });
      });
      this.nhs.run();
