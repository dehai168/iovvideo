...
class NodeRelayServer {
  ...
  putOneRTMP(conf) {
    ...
    if (this.staticSessions.has(conf.name)) {
      return;
    }
    ...
    session.id = conf.name;
    session.streamPath = `/${conf.app}/${conf.name}`;
    session.on('end', (id) => {
      this.staticSessions.delete(id);
    });
    this.staticSessions.set(conf.name, session);
    session.run();
    Logger.log('[Put one RTMP] start', conf.name, conf.inPath, ' to ', conf.ouPath);
  }
  onStatic() {
    if (!this.config.relay.tasks) {
      return;
